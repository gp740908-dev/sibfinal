...
export const VillaDetail: React.FC<VillaDetailProps> = ({ 
  ...
  // Use the passed onNavigate or fallback to window.location for simple navigation
  const handleNavigation = (view: string, id?: string) => {
    if (onNavigate) {
      onNavigate(view, id);
    } else if (view === 'villas') {
      window.location.href = '/#villas';
    } else if (view === 'villa-detail' && id) {
      // Force navigation for Next.js pages
    ...
    <div className="bg-sand min-h-screen pt-24 pb-20 animate-fade-in text-forest">
      ...
      <section className="px-4 md:px-12 max-w-7xl mx-auto mt-24 pt-12 border-t border-forest/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {similarVillas.map(v => (
            <div key={v.id} onClick={() => handleNavigation('villa-detail', v.id)}>
                 <VillaCard villa={v} />
            </div>
          ))}
        </div>
